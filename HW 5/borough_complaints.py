...
def parseFile(input_file, start_date, end_date, output_file):
    ...
    for chunk in pd.read_csv(input_file, usecols=[1, 5, 6, 7, 25], header=None, chunksize=chunksize):
    
        chunk.columns = ['created_date', 'complaint_type', 'complaint_desc', 'establishment', 'borough']
        ...
        # count occurences
        chunk_complaint_summary = filtered_chunk.groupby(['complaint_type', 'complaint_desc', 'establishment', 'borough']).size().reset_index(name='count')
        
        aggregated_complaints = pd.concat([aggregated_complaints, chunk_complaint_summary])

    # group again to get the final counts
    final_summary = aggregated_complaints.groupby(['complaint_type','complaint_desc', 'establishment', 'borough']).sum().reset_index()

    output_data = "complaint_type,complaint_desc,borough,count\n"
    for _, row in final_summary.iterrows():
        output_data += f"{row['complaint_type']},{row['complaint_desc']},{row['borough']},{row['establishment']},{row['count']}\n"
